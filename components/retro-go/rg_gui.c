...
void rg_gui_init(void)
{
    ...
    if (language_id == 3) // Old builds used 3 for Chinese before removing FR/DE
        language_id = RG_LANG_ZH;
    if (!rg_gui_set_language_id(language_id))
    {
        language_id = 0;
        rg_gui_set_language_id(language_id);
    }
    int font_type = rg_settings_get_number(NS_GLOBAL, SETTING_FONTTYPE, RG_FONT_DEFAULT);
#ifdef RG_TARGET_XIAOMIAO
    if (language_id == RG_LANG_ZH)
        font_type = RG_FONT_PINGFANG_ZH_12;
#endif
...
#ifdef RG_TARGET_XIAOMIAO
        if (gui.initialized && index == RG_LANG_ZH)
            rg_gui_set_font(RG_FONT_PINGFANG_ZH_12);
#endif
                           ...
                           rg_color_t color_fg, rg_color_t color_bg, uint32_t flags)
{
    ...
    if (!font)
    {
        RG_LOGE("Cannot draw text before a GUI font is configured");
...
intptr_t rg_gui_dialog(const char *title, const rg_gui_option_t *options_const, int selected_index)
{
    ...
    while (event != RG_DIALOG_SELECT && event != RG_DIALOG_CANCEL)
    {
        ...
        if (rg_input_menu_released(joystick))
        {
        ...
        else if (joystick ^ joystick_old)
        {
            ...
            else if (joystick & (RG_KEY_B|RG_KEY_OPTION)) {
                event = RG_DIALOG_CANCEL;
            }
