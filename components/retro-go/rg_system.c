...
static void system_monitor_task(void *arg)
{
    ...
    while (!exitCalled)
    {
        ...
        {
            // Only a fresh MENU press made after the timeout may terminate an unresponsive app.
            if (rg_input_wait_for_key(RG_KEY_MENU, false, 1000) &&
                rg_input_wait_for_key(RG_KEY_MENU, true, 1000))
            {
    ...
    RG_LOGW("System monitor task stopping");
}
...
rg_app_t *rg_system_init(int sampleRate, const rg_handlers_t *handlers, void *_unused)
{
...
#if defined(RG_I2C_GPIO_DRIVER) || defined(RG_RTC_PCF8563)
    rg_i2c_init();
#endif
    ...
    int bootButtons = rg_input_read_gamepad();
    RG_LOGI("Boot buttons=" PRINTF_BINARY_16 ", recovery mask=" PRINTF_BINARY_16 "\n",
        PRINTF_BINVAL_16(bootButtons), PRINTF_BINVAL_16(RG_RECOVERY_BTN));
    for (int timeout = 5, btn = bootButtons & RG_RECOVERY_BTN; btn && timeout >= 0; --timeout)
    {
        RG_LOGW("Recovery button " PRINTF_BINARY_16 " being held down...\n", PRINTF_BINVAL_16(btn));
        enterRecoveryMode = (timeout == 0);
        rg_task_delay(100);
        btn = rg_input_read_gamepad() & RG_RECOVERY_BTN;
    }
    ...
    if (app.lowMemoryMode)
        RG_LOGW("External memory not detected; continuing in low-memory mode.");
    ...
    rg_system_save_trace(RG_STORAGE_ROOT "/boot.log", false);
...
{
    rg_task_t *task = arg;
    QueueHandle_t queue = xQueueCreate(1, sizeof(rg_task_msg_t));
    task->handle = xTaskGetCurrentTaskHandle();
    task->queue = queue;
    if (!queue)
    {
        RG_LOGE("Task queue creation failed: name='%s'", task->name);
        memset(task, 0, sizeof(rg_task_t));
        vTaskDelete(NULL);
        return;
    }
    (task->func)(task->arg);
    RG_LOGW("Task returned: name='%s', queue='%p'", task->name, queue);
    vQueueDelete(queue);
    memset(task, 0, sizeof(rg_task_t));
    vTaskDelete(NULL);
}
...
#ifdef RG_RTC_PCF8563
#define PCF8563_ADDRESS  0x51
#define PCF8563_TIME_REG 0x02

static uint8_t time_to_bcd(int value)
{
    return (uint8_t)(((value / 10) << 4) | (value % 10));
}

static int time_from_bcd(uint8_t value)
{
    return (value >> 4) * 10 + (value & 0x0F);
}

static bool pcf8563_read_time(time_t *time_sec)
{
    uint8_t data[7];
    struct tm rtc_time = {0};
    struct tm checked_time;

    if (!rg_i2c_read(PCF8563_ADDRESS, PCF8563_TIME_REG, data, sizeof(data)))
        return false;
    if (data[0] & 0x80)
    {
        RG_LOGW("PCF8563 time is invalid (voltage-low flag set).\n");
        return false;
    }

    rtc_time.tm_sec = time_from_bcd(data[0] & 0x7F);
    rtc_time.tm_min = time_from_bcd(data[1] & 0x7F);
    rtc_time.tm_hour = time_from_bcd(data[2] & 0x3F);
    rtc_time.tm_mday = time_from_bcd(data[3] & 0x3F);
    rtc_time.tm_mon = time_from_bcd(data[5] & 0x1F) - 1;
    rtc_time.tm_year = time_from_bcd(data[6]) + 100;
    rtc_time.tm_isdst = -1;

    if (rtc_time.tm_sec > 59 || rtc_time.tm_min > 59 || rtc_time.tm_hour > 23 ||
        rtc_time.tm_mday < 1 || rtc_time.tm_mday > 31 ||
        rtc_time.tm_mon < 0 || rtc_time.tm_mon > 11)
        return false;

    checked_time = rtc_time;
    *time_sec = mktime(&checked_time);
    if (*time_sec == (time_t)-1 || checked_time.tm_year != rtc_time.tm_year ||
        checked_time.tm_mon != rtc_time.tm_mon || checked_time.tm_mday != rtc_time.tm_mday ||
        checked_time.tm_hour != rtc_time.tm_hour || checked_time.tm_min != rtc_time.tm_min ||
        checked_time.tm_sec != rtc_time.tm_sec)
        return false;

    return true;
}

static bool pcf8563_write_time(time_t time_sec)
{
    uint8_t data[7];
    struct tm local_time;

    if (!localtime_r(&time_sec, &local_time) || local_time.tm_year < 100 || local_time.tm_year > 199)
        return false;

    data[0] = time_to_bcd(local_time.tm_sec) & 0x7F;
    data[1] = time_to_bcd(local_time.tm_min) & 0x7F;
    data[2] = time_to_bcd(local_time.tm_hour) & 0x3F;
    data[3] = time_to_bcd(local_time.tm_mday) & 0x3F;
    data[4] = time_to_bcd(local_time.tm_wday) & 0x07;
    data[5] = time_to_bcd(local_time.tm_mon + 1) & 0x1F;
    data[6] = time_to_bcd(local_time.tm_year % 100);
    return rg_i2c_write(PCF8563_ADDRESS, PCF8563_TIME_REG, data, sizeof(data));
}
#endif

bool rg_system_set_time(time_t time_sec)
{
#ifdef ESP_PLATFORM
    if (settimeofday(&(struct timeval){time_sec, 0}, NULL) != 0)
        return false;
#endif
#ifdef RG_RTC_PCF8563
    if (!pcf8563_write_time(time_sec))
    {
        RG_LOGE("Failed to save time to PCF8563.\n");
        return false;
    }
    RG_LOGI("System time saved to PCF8563.\n");
#endif
    return true;
}
...
#ifdef RG_RTC_PCF8563
    if (pcf8563_read_time(&time_sec))
        RG_LOGI("Time loaded from PCF8563.\n");
    else
        RG_LOGW("Using build time because PCF8563 time is unavailable.\n");
#else
...
static void shutdown_cleanup(bool show_hourglass)
{
    exitCalled = true;
...
void rg_system_panic(const char *context, const char *message)
{
    ...
    rg_system_save_trace(RG_STORAGE_ROOT "/crash.log", true);
