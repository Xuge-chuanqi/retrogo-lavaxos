...
#if defined(RG_AUDIO_FORCE_EXT_DAC) && RG_AUDIO_FORCE_EXT_DAC && RG_AUDIO_USE_EXT_DAC
    // Migrate a previously saved Dummy selection to the external I2S DAC.
    if (audio.sink == &sinks[0])
    {
        for (size_t i = 1; i < RG_COUNT(sinks); ++i)
        {
            if (sinks[i].driver == &rg_audio_driver_i2s && sinks[i].device == 1)
            {
                audio.sink = &sinks[i];
