...
static bool system_combo_held = false;
...
bool rg_input_read_gamepad_raw(uint32_t *out)
{
    ...
    for (size_t i = 0; i < RG_COUNT(keymap_gpio); ++i)
    {
        ...
        int level = gpio_get_level(mapping->num);
...
#ifndef RG_GAMEPAD_SERIAL_BITS
#define RG_GAMEPAD_SERIAL_BITS 16
#endif
    ...
    rg_usleep(1);
    uint32_t buttons = 0;
    for (int i = 0; i < RG_GAMEPAD_SERIAL_BITS; i++)
    {
        gpio_set_level(RG_GPIO_GAMEPAD_CLOCK, 0);
        rg_usleep(1);
        buttons |= gpio_get_level(RG_GPIO_GAMEPAD_DATA) << (RG_GAMEPAD_SERIAL_BITS - 1 - i);
...
void rg_input_init(void)
{
    ...
    gpio_config_t serial_output_config = {
        .pin_bit_mask =
            (1ULL << RG_GPIO_GAMEPAD_CLOCK) |
            (1ULL << RG_GPIO_GAMEPAD_LATCH),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    };
    ESP_ERROR_CHECK(gpio_config(&serial_output_config));

    gpio_config_t serial_input_config = {
        .pin_bit_mask = 1ULL << RG_GPIO_GAMEPAD_DATA,
        .mode = GPIO_MODE_INPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    };
    ESP_ERROR_CHECK(gpio_config(&serial_input_config));
...
bool rg_input_menu_released(uint32_t keys)
{
    uint32_t combo = keys & (RG_KEY_MENU | RG_KEY_SELECT | RG_KEY_START);
    ...
    if (system_combo_held)
    {
        if (combo == 0)
            system_combo_held = false;
    }
    else if (combo == (RG_KEY_MENU | RG_KEY_SELECT))
    {
        system_combo_held = true;
        rg_display_toggle_backlight();
    }
    else if (combo == (RG_KEY_MENU | RG_KEY_START))
    {
        system_combo_held = true;
        rg_system_sleep();
    }
